...
if [ "$#" -ne 5 ]; then
    echo "Usage: $0 <firmware_version> <ap_name> <band (2G|5G|6G)> <start_ip_octet 2-9> <end_ip_octet 2-9>"
    exit 1
fi
...
# Validate that inputs are integers between 2 and 9
if ! [[ "$START_OCTET" =~ ^[2-9]$ ]] || ! [[ "$END_OCTET" =~ ^[2-9]$ ]]; then
    echo "Error: Both, start octet and end octet must be integers between 2 and 9."
fi

# Ensure start is not greater than end
if (( $START_OCTET > $END_OCTET)); then
    echo "Error: Start octet must be less than or equal to end octet."
fi

# Build client ip adress list
